package com.scetia.Pro.common.exceptionhandle;
...
public class ExceptionHandle {
     ...
     * 本地错误
     */
    public static final int LOCAL_ERROR = 1006;

    private static final int UNAUTHORIZED = 401;
    private static final int FORBIDDEN = 403;
    private static final int NOT_FOUND = 404;
    //提交失败
    private static final int POST_FAILURE = 422;
    ...
    public static ResponseThrowable handleException(Throwable e) {
        ...
        if (e instanceof HttpException) {
            ...
            ex = handlerBuildSandException(httpException);
            if(ex!=null){
                return ex;
            }
            ...
            ex = new ResponseThrowable(e, NETWORD_ERROR);
     ...
     * @return
     */
    private static ResponseThrowable handlerBuildSandException(HttpException exception){
        ResponseThrowable responseThrowable=null;
        ResponseBody responseBody = exception.response().errorBody();
        if(responseBody!=null){
            try {
                String string = responseBody.string();
                if(!string.contains("responseException")) {
                    return null;
                }
                ...
                responseThrowable = new ResponseThrowable(exception,exception.code());
                responseThrowable.message = jsonElement.toString();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
