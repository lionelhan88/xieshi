...
public class LearnDataActivity extends NavigationActivity {
    ...
    private void getCourseScores(PushToDx pushToDx){
        ...
        final Observable<TrainingResultData<List<CourseScore>>>[] observables = new Observable[1];
        final String userId = pushToDx.getUserId();
        Observable.fromArray(projects)
                .flatMap(new Function<List<Project>, ObservableSource<TrainingResultData<List<CourseScore>>>>() {
                    @Override
                    public ObservableSource<TrainingResultData<List<CourseScore>>> apply(List<Project> projects) throws Exception {
                        List<Observable<TrainingResultData<List<CourseScore>>>> arrs = new ArrayList<>();
                ...
                .subscribe(new ApiObserver<TrainingResultData<List<CourseScore>>>() {
                    @Override
                    public void success(TrainingResultData<List<CourseScore>> courseScoreTrainingResultData) {
                        List<CourseScore> records = courseScoreTrainingResultData.getRecord();
                        //有数据时才加入集合
                        if(records.size()>0) {
