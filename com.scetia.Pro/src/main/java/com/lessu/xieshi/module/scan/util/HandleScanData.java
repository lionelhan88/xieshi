...
public class HandleScanData {
     ...
     * @param callBackListener
     */
    public void startReadingByNetWork2(CallBackListener callBackListener) {
        Observable.create((ObservableOnSubscribe<Object>) emitter -> {
            String receiveData = "";
            String receiveDataFirst = "";
            String receiveDataSecond = "";
            ...
            while (isReading) {
                try {
                    ...
                    if (datas.matches("[0-9]*")) {
                        ...
                        String s = LongString.bytes2HexString(buffer2);
                        receiveData = Decrypt.decodeChip(s);
                     /*   这里也要注意，读取芯片时会多次读取，为了防止多次重复访问服务，这里记录oldData如果和当前
                        读取的芯片字符串相同，则跳过不再请求网络数据*/
                        if (oldData.equals(receiveData)) {
                            receiveData = "";
                            receiveDataFirst = "";
                    ...
                    if (receiveData != null && receiveData.length() == 10) {
                        //识别到的id
                        String finalReceiveData = receiveData;
                        emitter.onNext("正在识别...,"+finalReceiveData);
                        ReceiveSampleInfoBean readData = getReadData2(finalReceiveData);
                        if (readData == null) {
                            emitter.onNext("未能获取数据,"+finalReceiveData);
                            //旧数据清空
                            oldData = "";
                            emitter.onNext(new ReceiveSampleInfoBean());
                        } else {
                            readData.setCodeNumber(receiveData);
                            emitter.onNext("识别完成,"+finalReceiveData);
                            emitter.onNext(readData);
                            //记录旧数据
                            oldData = receiveData;
                        }
    ...
    private ReceiveSampleInfoBean getReadData(String s){
        ...
        return  parseInfoBean(soap3);
    }

    private ReceiveSampleInfoBean getReadData2(String s) {
        String nameSpace1 = "http://tempuri.org/";
     ...
     * @return
     */
    private ReceiveSampleInfoBean parseInfoBean(SoapObject soap3){
    ...
    public static String formatData(byte[] buffer){
        String datas;
        if(buffer[0]==-86){
            datas=new String(buffer, 1, buffer.length-1);
        }else{
            datas= new String(buffer, 0, buffer.length);
        }
