                ...
                if (jsonArray.size() > 0) {
                    ...
                    try {
                        ...
                        for (int i = 0; i < paramInfoJson.size(); i++) {
                            ...
                            htmlString.append(String.format("<li class=\"detail-switch-li\"><p class=\"detail-switch-title\">%s</p><p class=\"detail-switch-content\">%s(检测:%s，标准:%s)</p></li>",
                                    parameterName, uCResultValue, uCTestValue, uCStandardValue));
                        }
                        json.getAsJsonObject().addProperty("ParamInfoHtml", htmlString.toString());
                    } catch (Exception e) {
